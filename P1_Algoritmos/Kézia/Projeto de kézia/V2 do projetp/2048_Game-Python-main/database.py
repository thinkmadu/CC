...
def get_best(count: int = 0) -> dict[int: tuple[None | str], int]:
    """ Retorna com o top3 jogadores. """
    try:
        assert -1 <= count <= 3
    except AssertionError:
        raise ValueError('Invalid argument count, must be no more than 3 and no less than -1')
    cursor.execute("SELECT name, max(score) score FROM RECORDS GROUP by name ORDER by score DESC LIMIT 3")
...
# isso daqui é uma variável de conexão: pra parte do código e a base de dados em si ficarem conectadas
