...
cursor.execute("CREATE TABLE ranking(nome text, score integer)")
cursor.execute("INSERT INTO ranking VALUES('mf', 420)")
banco.commit()
'''
...
"""
def top3(count: int = 0) -> dict[int: tuple[None | str], int]: #9
    try: #10
        assert -1 <= count <= 3
    except AssertionError:
        raise ValueError('Invalid argument count, must be no more than 3 and no less than -1')
    cursor.execute("SELECT nome, max(score) score FROM ranking GROUP by nome ORDER by score DESC LIMIT 3")  #11
...
"""
def inserirResultado(nome: str, score: int) -> None: #14
    cursor.execute(nome, score)
    banco.commit() #15 
